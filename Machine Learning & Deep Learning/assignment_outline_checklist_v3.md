...
- [ ] Define [0, 1] normalisation function (divide by 255) — for Custom CNN and SVM
- [ ] Define ImageNet normalisation function — for Transfer Learning only
- [ ] Ensure val and test sets use the same normalisation as training for each model

#### 4.4.3 Data Augmentation (training split only)
- [ ] Expand training set at 5× with: flip, rotate, brightness, crop, Gaussian noise
- [ ] Report dataset sizes before and after augmentation
